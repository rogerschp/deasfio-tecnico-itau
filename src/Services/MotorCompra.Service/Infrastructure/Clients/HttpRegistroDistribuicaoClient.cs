...
public class HttpRegistroDistribuicaoClient : IRegistroDistribuicaoClient
{
    ...
    public async Task RegistrarDistribuicaoAsync(long clienteId, long execucaoId, IReadOnlyList<ItemDistribuicaoDto> itens, CancellationToken ct = default)
    {
        if (itens.Count == 0) return;
        var payload = new { execucaoId, clienteId, itens };
